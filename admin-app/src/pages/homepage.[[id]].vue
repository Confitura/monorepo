...
const presentations = ref<InlinePresentation[]>([])
const workshops = ref<InlineWorkshop[]>([])
const profile = ref<User>()
const cospeakers = ref<User[]>([])
const newCospeakerEmail = ref('')
const selectedPresentation = ref<InlinePresentation | null>(null)
const selectedWorkshop = ref<InlineWorkshop | null>(null)
const cospeakersDialogVisible = ref(false)
...
function loadCospeakers(id: string, workshop: boolean = false) {
  ...
  if (workshop) {
    ...
    selectedPresentation.value =
      presentations.value.find((p) => p.id === id) || null
...
function deleteItem(
  item: InlinePresentation | InlineWorkshop,
...
<template>
        ...
        <v-card v-if="profile" class="mx-auto">
          <v-card-actions>
            <v-list-item class="w-100">
              <template #prepend>
                <v-avatar color="grey-darken-3" :image="profile.photo" />
              </template>
          ...
          <v-list>
            ...
            <v-list-item>
              <template #prepend>
                <v-icon icon="mdi-alpha-x" />
              </template>
              <v-list-item-title v-text="profile.twitter" />
            </v-list-item>
            <v-list-item>
              <template #prepend>
                <v-icon icon="mdi-github" />
              </template>
              <v-list-item-title v-text="profile.github" />
            </v-list-item>
            <v-list-item>
              <template #prepend>
                <v-icon icon="mdi-web" />
              </template>
              <v-list-item-title v-text="profile.www" />
            </v-list-item>
          </v-list>
        </v-card>
      </v-col>
                ...
                <v-card-actions>
                  ...
                  <v-btn color="medium-emphasis" icon="mdi-delete" size="small">
                    ...
                    <v-menu activator="parent">
                      <v-btn @click="deleteItem(workshop, true)">
                        YES, DELETE
                      </v-btn>
                    </v-menu>
                  </v-btn>
              ...
              >
                <v-card-actions>
                  ...
                  <v-btn color="medium-emphasis" icon="mdi-delete" size="small">
                    ...
                    <v-menu activator="parent">
                      <v-btn @click="deleteItem(presentation)">
                        YES, DELETE
                      </v-btn>
                    </v-menu>
                  </v-btn>
  ...
  <v-dialog v-model="cospeakersDialogVisible" max-width="600px">
    <v-card v-if="selectedPresentation || selectedWorkshop">
      ...
      <v-card-text>
        <v-list>
          ...
          <v-list-item v-for="cospeaker in cospeakers" :key="cospeaker.id">
            <v-list-item-title
              >{{ cospeaker.name }} ({{ cospeaker.email }})
            </v-list-item-title>
            <template #append>
              <v-btn
                color="error"
          ...
          <v-form
            @submit.prevent="
              addCospeaker(
                isWorkshop ? selectedWorkshop!.id! : selectedPresentation!.id!,
              )
            "
          >
            <v-text-field
              ...
              required
            />
            <v-btn color="primary" type="submit" :disabled="!newCospeakerEmail">
              Add Cospeaker
            </v-btn>
          </v-form>
        </v-list>
      ...
      <v-card-actions>
        <v-spacer />
        <v-btn color="primary" @click="cospeakersDialogVisible = false"
          >Close
        </v-btn>
      </v-card-actions>
    </v-card>
