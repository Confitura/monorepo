...
class RatingApiTest extends BaseIntegrationTest {
    ...
    public void userShouldBeAbleToUpdateRate() throws Exception {
        ...
        ResultActions resultActions = mockMvc.perform(get("/presentations/" + presentationId + "/ratings"))
                .andExpect(status().isOk());
        resultActions
                .andExpect(jsonPath("$").value(hasSize(1)))
    ...
    public void userShouldBeAbleToRateSecondPresentation() throws Exception {
        ...
        String presentationId = agenda.get(1).getPresentationId();
        Presentation secondPresentation = presentationFacade.findById(presentationId);

        //when second presentaion is rated
        rate(presentationId, user, secondRate);
    ...
    private ResultActions rate(String presentationId, User user, Rate rate) throws Exception {
