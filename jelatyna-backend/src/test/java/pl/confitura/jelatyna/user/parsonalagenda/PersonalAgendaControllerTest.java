...
class PersonalAgendaControllerTest extends BaseIntegrationTest {
    ...
    public void setUp() {
        ...
        entriesByPresentationTitle = agenda.stream()
                ...
                .collect(
                        toMap(
                                agendaEntry -> {
                                    String presentationId = agendaEntry.getPresentationId();
                                    Presentation presentation = presentationFacade.findById(presentationId);
    ...
    public void personalAgendaShouldContainOnlyOneEntryForGivenTimeSlot() throws Exception {
        ...
        ResultActions perform = mockMvc.perform(get("/users/" + user.getId() + "/personalAgenda"));
        perform
